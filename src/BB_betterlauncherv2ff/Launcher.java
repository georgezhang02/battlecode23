...
public strictfp class Launcher {
    ...
    static RobotInfo[] alliesPrevious;
    ...
    static void checkMovement(RobotController rc){
        for(int i = 0; i < allies.length; i++){
            if(allies[i].getType() == RobotType.LAUNCHER){
                ...
                MapLocation loc = allies[i].getLocation();
                for(int j = 0; j < alliesPrevious.length; j++){
                    //If the ids match up
                    if(alliesPrevious[j] != null && ID == alliesPrevious[j].getID()){
        ...
        if (canExplore) {
            ...
            if(numNearbyAllyMil>=2){
                ...
                if(movementChange){
                    ...
                    rc.setIndicatorString(followBot.getLocation().toString());
                }
                else{
