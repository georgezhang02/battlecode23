...
public strictfp class Carrier {
    ...
    private enum CarrierState {
        None, Exploring, Returning, Anchoring, Gathering, Runaway, Converting
    }
    ...
    //CURRENTLY ARE SET EQUAL TO CLOSESTAD/MN
    static MapLocation convertAD;
    static MapLocation closestMN;

    static MapLocation convertMN;
    ...
    static void run(RobotController rc) throws GameActionException {
        ...
        //For now, all carriers considered "converters"
        converter = true;
        readComms(rc);
        if(!initialized){
    ...
    static void gatherUpdate(RobotController rc) throws GameActionException {
        ...
        if(adAmount + manaAmount + elixirAmount == 40){
            ...
            //If it has ad and the round is between 500 and 700, convert to elixir
            if(rc.getRoundNum() > 500 && rc.getRoundNum() < 700 && adAmount == 40 && converter){
    ...
    static void convertUpdate(RobotController rc) throws GameActionException{
        if(convertMN == null){
            state = CarrierState.Gathering;
            return;
        }
        if(rc.getLocation().isAdjacentTo(convertMN)){
            if(rc.canTransferResource(convertMN, ResourceType.ADAMANTIUM, 40)){
                rc.transferResource(convertMN, ResourceType.ADAMANTIUM, 40);
                adAmount = 0;
            }
            state = CarrierState.Gathering;
        }
