...
public strictfp class Carrier {
    ...
    private enum CarrierState {
        None, Exploring, Returning, Anchoring, Gathering, Converting, Runaway
    }
    ...
    static MapLocation oppositeWell = new MapLocation(14,20);
    ...
    static void gatherUpdate() {
        ...
        else if(adAmount + manaAmount + elixirAmount == 40){
            //state = CarrierState.Returning;
            state = CarrierState.Converting;
        }
    }
    ...
    static void convertUpdate(RobotController rc){
        WellInfo[] well = rc.senseNearbyWells();
        if(well.length > 0 && well[0].getResourceType() == ResourceType.ELIXIR){
            assignedWell = well[0].getMapLocation();
            state = CarrierState.Gathering;
        }
        else if (oppositeWell.distanceSquaredTo(location) <= 2
                && adAmount == 0 && manaAmount == 0 && elixirAmount == 0) {
                state = CarrierState.Gathering;
        }
    }

    static void convert(RobotController rc) throws GameActionException{
        if(rc.getHealth() < RobotType.CARRIER.getMaxHealth()) {
            for (RobotInfo enemy : enemies) {
                if (rc.canAttack(enemy.getLocation())) {
                    rc.attack(enemy.getLocation());
        ...
        //move back to the opposite well
        if(oppositeWell.distanceSquaredTo(location) > 2){
            pathTowards(rc, oppositeWell);
        }

        //If next to the well, deposit resources to well to convert
        if (oppositeWell.distanceSquaredTo(location) <= 2){
            if(rc.canTransferResource(oppositeWell, ResourceType.MANA, manaAmount) && manaAmount != 0){
                rc.transferResource(oppositeWell,ResourceType.MANA, manaAmount);
            }
            else if(rc.canTransferResource(oppositeWell, ResourceType.ADAMANTIUM, adAmount) && adAmount != 0){
                rc.transferResource(oppositeWell,ResourceType.ADAMANTIUM, adAmount);
            }
        }
    }
    /*
